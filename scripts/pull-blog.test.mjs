...
const scriptPath = path.join(path.dirname(fileURLToPath(import.meta.url)), "pull-blog.mjs");
...
function makeFixtureRepo(name, files) {
  const repoDir = mkdtempSync(path.join(os.tmpdir(), `blog-fixture-${name}-`));
...
// touch the real blog-sources.json or src/content/blog.
function makeProject(sources) {
  const projectDir = mkdtempSync(path.join(os.tmpdir(), "blog-project-"));
  writeConfig(projectDir, sources);
  return projectDir;
}

function writeConfig(projectDir, sources) {
  writeFileSync(path.join(projectDir, "blog-sources.json"), JSON.stringify(sources, null, 2));
}

// Runs the script exactly as npm run pull-blog does: as a child process with
...
function blogDir(projectDir) {
  return path.join(projectDir, "src", "content", "blog");
}

test("pulls every markdown file from every listed repo into src/content/blog/<repoName>/<path>/", (t) => {
  ...
  assert.equal(readFileSync(path.join(blogDir(project), alphaName, "notes", "one.md"), "utf8"), "# one\n");
  assert.equal(readFileSync(path.join(blogDir(project), alphaName, "notes", "two.md"), "utf8"), "# two\n");
  assert.equal(readFileSync(path.join(blogDir(project), betaName, "posts", "three.md"), "utf8"), "# three\n");
  assert.equal(existsSync(path.join(blogDir(project), alphaName, "README.md")), false);
});

test("adding a second config entry brings its files in with no other edit", (t) => {
  const alpha = makeFixtureRepo("alpha", { "notes/one.md": "# one\n" });
  const beta = makeFixtureRepo("beta", { "posts/two.md": "# two\n" });
  const project = makeProject([{ repo: alpha.url, path: "notes" }]);
  t.after(() => {
    rmSync(alpha.repoDir, { recursive: true, force: true });
    rmSync(beta.repoDir, { recursive: true, force: true });
    rmSync(project, { recursive: true, force: true });
  ...
  assert.equal(existsSync(path.join(blogDir(project), path.basename(beta.repoDir))), false);
  ...
  assert.equal(result.status, 0, `expected success, stderr: ${result.stderr}`);
  assert.ok(existsSync(path.join(blogDir(project), path.basename(alpha.repoDir), "notes", "one.md")));
  assert.ok(existsSync(path.join(blogDir(project), path.basename(beta.repoDir), "posts", "two.md")));
});
...
test("removing a config entry and re-running removes its files", (t) => {
  ...
  writeConfig(project, [{ repo: alpha.url, path: "notes" }]);
  const result = runScript(project);
...
test("a clone failure exits nonzero and names the entry", (t) => {
  const missingUrl = pathToFileURL(path.join(os.tmpdir(), "blog-no-such-repo")).href;
  const project = makeProject([{ repo: missingUrl, path: "notes" }]);
...
test("malformed JSON in blog-sources.json exits nonzero naming the config", (t) => {
  const project = mkdtempSync(path.join(os.tmpdir(), "blog-project-"));
  writeFileSync(path.join(project, "blog-sources.json"), "{ not json");
  t.after(() => rmSync(project, { recursive: true, force: true }));
...
test("a config that is not an array exits nonzero", (t) => {
  const project = mkdtempSync(path.join(os.tmpdir(), "blog-project-"));
  writeFileSync(path.join(project, "blog-sources.json"), JSON.stringify({ repo: "a/b", path: "c" }));
  t.after(() => rmSync(project, { recursive: true, force: true }));
...
test("symlinks in a source repo are not copied into src/content/blog", (t) => {
  const alpha = makeFixtureRepo("alpha", { "notes/one.md": "# one\n" });
  // A symlink committed to the repo could point anywhere on the machine that
  // clones it; the pull must copy only regular files.
  symlinkSync("/etc/hostname", path.join(alpha.repoDir, "notes", "leak.md"));
  ...
  assert.equal(result.status, 0, `expected success, stderr: ${result.stderr}`);
  const alphaName = path.basename(alpha.repoDir);
  assert.ok(existsSync(path.join(blogDir(project), alphaName, "notes", "one.md")));
  assert.equal(existsSync(path.join(blogDir(project), alphaName, "notes", "leak.md")), false);
});

test('a config path with ".." or an absolute path exits nonzero without cloning', (t) => {
  for (const badPath of ["../outside", "notes/../../outside", "/etc"]) {
